        ...
        with tf.name_scope("layer_biases"):
            if isinstance(bias, float):
                bias = tf.get_variable(
                    "biases", [num_filters], initializer=tf.constant_initializer(bias)
                )
...
def __depthwise_conv2d_p(
    ...
    with tf.variable_scope(name):
        ...
        with tf.name_scope("layer_biases"):
            if isinstance(bias, float):
                bias = tf.get_variable(
                    "biases", [x.shape[-1]], initializer=tf.constant_initializer(bias)
                )
            __variable_summaries(bias)
        with tf.name_scope("layer_conv2d"):
...
def __dense_p(
    ...
    with tf.variable_scope(name):
        if w == None:
            w = __variable_with_weight_decay(
                [n_in, output_dim], initializer, l2_strength
            )
        __variable_summaries(w)
        if isinstance(bias, float):
            bias = tf.get_variable(
                "layer_biases", [output_dim], tf.float32, tf.constant_initializer(bias)
            )
        __variable_summaries(bias)
